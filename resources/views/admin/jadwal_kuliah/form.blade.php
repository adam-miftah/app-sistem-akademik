  ...
  <div class="col-lg-8">
    <div class="card shadow-sm border-0">
      <div class="card-header bg-white p-3">
        <h5 class="mb-0 fw-semibold"><i class="fas fa-calendar-plus me-2 text-primary"></i>Detail Jadwal</h5>
      </div>
      <div class="card-body">
        <div class="row g-3">
          <div class="col-12">
          ...
          {{ $mk->nama_mk }} ({{$mk->sks}} SKS) - {{ $mk->kelas }}
          </option>
        @endforeach
            </select>
          </div>
          <div class="col-12">
            <label for="dosen_id" class="form-label required">Dosen Pengampu</label>
        ...
        @endforeach
          ...
          <div class="col-md-6"><label for="hari" class="form-label required">Hari</label><select id="hari" name="hari"
              class="form-select" required>@foreach($haris as $hari)<option value="{{ $hari }}" {{ old('hari', $jadwalKuliah->hari ?? '') == $hari ? 'selected' : '' }}>{{ $hari }}</option>@endforeach</select></div>
          <div class="col-md-6"><label for="ruangan" class="form-label required">Ruangan</label><input type="text"
              ...
              value="{{ old('ruangan', $jadwalKuliah->ruangan ?? '') }}" required></div>
          <div class="col-md-6"><label for="jam_mulai" class="form-label required">Jam Mulai</label><input type="time"
              ...
              value="{{ old('jam_mulai', $jadwalKuliah->jam_mulai ? \Carbon\Carbon::parse($jadwalKuliah->jam_mulai)->format('H:i') : '') }}"
              required></div>
          <div class="col-md-6"><label for="jam_selesai" class="form-label required">Jam Selesai</label><input
              ...
              value="{{ old('jam_selesai', $jadwalKuliah->jam_selesai ? \Carbon\Carbon::parse($jadwalKuliah->jam_selesai)->format('H:i') : '') }}"
              required></div>
          <div class="col-12"><label for="kelas" class="form-label required">Kelas</label><select id="kelas"
              ...
              <option value="">Pilih Kelas...</option>@foreach($kelasOptions as $option)<option value="{{ $option }}" {{ old('kelas', $jadwalKuliah->kelas ?? '') == $option ? 'selected' : '' }}>{{ $option }}</option>@endforeach
            </select></div>
        </div>
      </div>
    </div>
    ...
    <div class="card shadow-sm border-0 sticky-top" style="top: 20px;">
      ...
      <div class="card-body">
        ...
        <div class="d-grid gap-2">
          ...
          <a href="{{ route('admin.jadwalKuliahs.index') }}" class="btn btn-outline-secondary">Batal</a>
        </div>
      </div>
    </div>
  </div>
